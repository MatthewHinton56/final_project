...
DIVISIONS = eval(sys.argv[4])
thread_scale = eval(sys.argv[5])
THREAD_N = 32 * thread_scale
BUCKET_BANDWIDTH = 128
BLOCK_N = (SIZE / (THREAD_N * BUCKET_BANDWIDTH)) / thread_scale
static_shared_memory = 4 * DIVISIONS
#print(config_selection)
#for config in config_selection:
  #print(config)
#print(BLOCK_MEMORY // THREAD_N)
#print(SIZE // (THREAD_N * BLOCK_N))
#print (BIN_COUNT // THREAD_N)
simulate_kernel('bucketcount', [SIZE / (THREAD_N * BLOCK_N), math.log(DIVISIONS/4, 2), DIVISIONS / THREAD_N], config_selection, THREAD_N, BLOCK_N, .5)
