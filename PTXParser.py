...
def ptx_parse_function(ptx_file, kernel_name, loop_iterations_counts, gpu_generation):
    provided_loops = len(loop_iterations_counts)
    kernel_instructions = get_kernel_insts(ptx_file, kernel_name)
    try:
       alu_latencies = get_alu_latencies(getattr(sys.modules[__name__], gpu_generation))
